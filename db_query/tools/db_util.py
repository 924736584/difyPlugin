...
class DbUtil:
    ...
    def __init__(self, db_type: str,
        ...
        if db_type=='oracle':
            ...
            quoted_dsn = parse.quote_plus(dsn)
            connect_str = f"oracle+cx_oracle://{self.username}:{self.password}@/?dsn={quoted_dsn}"
            if self.is_not_empty(self.properties):
                connect_str += f"&{self.properties}"
            logging.info(f"Oracle connect str: {connect_str}")
            self.engine = create_engine(connect_str, pool_size=100, pool_recycle=36)
        else:  
